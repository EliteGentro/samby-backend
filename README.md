...
Samby Guide conversations are also durable in Neon and scoped to the authorized workspace member or guest. The guide uses the model selected by `AI_PROVIDER` (`openrouter` or `gemini`), retrieves page-relevant product behavior, and runs numerical workspace questions through deterministic metric tools. Configure `AI_PROVIDER_FALLBACK` to retry a different provider only when the primary returns HTTP 403; each provider keeps its own configured model. Configure the matching provider keys and models (`OPENROUTER_API_KEY` / `OPENROUTER_MODEL` and `GEMINI_API_KEY` / `GEMINI_MODEL`). Voice mode synthesizes saved guide answers through ElevenLabs with the server-only `ELEVEN_LABS_API_KEY`; the default voice and model can be changed with `ELEVEN_LABS_VOICE_ID` and `ELEVEN_LABS_MODEL`. See [`docs/assistant-architecture.md`](docs/assistant-architecture.md) for the request flow, access boundary, and API routes.
